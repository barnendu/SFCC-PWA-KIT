...
module.exports = {
    app: {
        ...
        // Configuration for the ACI Worldwide gateway.  These values are consumed by
        // the server-side routes implemented in overrides/app/ssr.js when the
        // frontend requests a new checkout and later queries the payment result.
        aciGateway: {
            endpoint: process.env.ACI_GATEWAY_ENDPOINT || '',
            entityId: process.env.ACI_ENTITY_ID || '',
            merchantId: process.env.ACI_MERCHANT_ID || '',
            // Set to a full Authorization header value, e.g. "Bearer <token>"
            authHeader: process.env.ACI_AUTH_HEADER || '',
            // Optional override for the widget script URL (if different from endpoint)
            widgetScriptUrl: process.env.ACI_WIDGET_SCRIPT_URL || '',
            // Default payment type used for checkout creation (ACI uses "DB" for debit/capture)
            paymentType: process.env.ACI_PAYMENT_TYPE || 'DB',
            // Brands shown in the widget
            brands: process.env.ACI_WIDGET_BRANDS || 'VISA MASTER',
            // Relative path used as shopperResultUrl
            resultPath: process.env.ACI_RESULT_PATH || '/aci-result',
            // any other credentials you require (api key, secret, etc.)
