...
import {
    createAciCheckoutController,
    fetchAciPaymentResultController
} from './aci/aci-controller'
import {getAciCspOrigins} from './aci/aci-service'
...
const {handler} = runtime.createHandler(options, (app) => {
    ...
    const aciOriginList = getAciCspOrigins(config.app?.aciGateway || {})
    ...
    if (aciOriginList.length) {
        contentSecurityPolicy.directives['script-src'].push(...aciOriginList)
        contentSecurityPolicy.directives['connect-src'].push(...aciOriginList)
        contentSecurityPolicy.directives['frame-src'].push(...aciOriginList)
    }
    ...
    // endpoint used by the frontend override (see overrides/app/pages/checkout/partials/payment.jsx)
    // it creates an ACI checkout ID and returns the data needed to render the widget.
    app.post('/api/aci/create-checkout', async (req, res) => {
        createAciCheckoutController(req, res, config)
    })

    // endpoint used by the ACI result page. It queries the payment result on the
    // ACI API and returns the normalized response to the frontend.
    app.post('/api/aci/payment-result', async (req, res) => {
        fetchAciPaymentResultController(req, res, config)
    })
